    ...
    public class WP_Client
    {
        ...
        public Boolean Connect(string IP)  //连接函数     无参数
        {
            ...
            if (mySocket != null)
            {
                ...
                connArg.RemoteEndPoint = new DnsEndPoint(IP, 6688);
        ...
        public void SendCommand(byte[] txt)     //发送函数    txt为发送你容
        {
            if (mySocket != null && mySocket.Connected)
            {
                SocketAsyncEventArgs sendArg = new SocketAsyncEventArgs();
                byte[] buffer = txt;
                Debug.WriteLine("一百"+buffer.Length.ToString(), DateTime.Now.ToShortTimeString());
                sendArg.SetBuffer(buffer, 0, buffer.Length);
                sendArg.Completed += (objSender, mArg) =>
                {
                    if (mArg.SocketError == SocketError.Success)
        ...
        public byte[] Receive(int size, int meiyong)   //接收函数       response为接收内容
        {
            if (size == 0) { size = 10; }
